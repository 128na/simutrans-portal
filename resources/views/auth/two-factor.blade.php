...
<div class="mx-auto max-w-xl p-6 lg:px-8">
    ...
    <form action="{{route('two-factor.login.store')}}" method="POST">
        ...
        <div class="flex flex-col gap-y-4 border-t border-gray-200 pt-6 lg:mx-0">
            ...
            <div>
                <label for="recovery_code" class="block text-sm/6 font-semibold text-primary">リカバリコード</label>
                <p class="mt-2 text-sm text-secondary">
                    認証コードが利用できないときは、二要素認証登録時に発行したリカバリコードを使用してください。
                </p>
                <div class="mt-2.5">
                    <input id="recovery_code" type="recovery_code" name="recovery_code" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-primary outline-1 -outline-offset-1 outline-gray-300 placeholder:text-muted focus:outline-2 focus:-outline-offset-2 focus:outline-brand sm:w-128" />
                </div>
                @error('recovery_code')
                <div class="text-sm text-red-600">{{$message}}</div>
                @enderror
