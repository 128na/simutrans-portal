...
<div class="form-group">
    <label for="author"><span class="badge badge-secondary mr-1">{{ __('Optional') }}</span>{{ __('Author') }}</label>
    <input type="text" class="form-control" id="author" name="author" value="{{ old('author', $article->author ?? '') }}">
    <small class="form-text text-muted">{{ __('If empty, the username is used.') }}</small>
</div>

<div class="form-group">
    <label><span class="badge badge-danger mr-1">{{ __('Required') }}</span>{{ __('Addon File') }} </label>
    <div class="mb-2">
        <p id="file_preview">
...
<div class="form-group">
    <label for="description"><span class="badge badge-danger mr-1">{{ __('Required') }}</span>{{ __('Description') }}</label>
