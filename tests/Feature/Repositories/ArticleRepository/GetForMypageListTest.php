...
final class GetForMypageListTest extends TestCase
{
    ...
    public function test(): void
    {
        ...
        $result = $this->articleRepository->getForMypageList($this->user);
    ...
    public function test下書き記事も取得される(): void
    {
        // 公開記事
        $publishedArticle = $this->createPage($this->user);

        // 下書き記事
        $draftArticle = Article::factory()->page()->draft()->create(['user_id' => $this->user->id]);

        $result = $this->articleRepository->getForMypageList($this->user);

        $this->assertCount(2, $result, '公開・非公開問わず取得されること');
    }
    ...
    public function test必要なカラムのみ取得(): void
    {
        ...
        $this->assertNotNull($firstArticle->slug);
        $this->assertNotNull($firstArticle->status);
        $this->assertNotNull($firstArticle->post_type);
