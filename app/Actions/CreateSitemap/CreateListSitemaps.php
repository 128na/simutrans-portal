...
final class CreateListSitemaps
{
    ...
    {
        yield 'user.xml' => User::has('articles')
            ->cursor()
            ->map(fn(User $user) => route('user', ['userIdOrNickname' => $user->nickname ?? $user->id]));
        yield 'category.xml' => Category::has('articles')
            ->cursor()
            ->map(fn(Category $category) => route('category', [
                'type' => $category->type->value,
                'slug' => $category->slug,
            ]));
        yield 'tag.xml' => Tag::has('articles')->cursor()->map(fn(Tag $tag) => route('tag', $tag));
