...
// 一覧系
Route::get('/pak128-japan', [\App\Http\Controllers\Front\ArticleController::class, 'pak128jp'])->name('pak.128japan');
Route::get('/pak128', [\App\Http\Controllers\Front\ArticleController::class, 'pak128'])->name('pak.128');
Route::get('/pak64', [\App\Http\Controllers\Front\ArticleController::class, 'pak64'])->name('pak.64');
Route::get('/pak-others', [\App\Http\Controllers\Front\ArticleController::class, 'pakOthers'])->name('pak.others');
Route::get('/search', [\App\Http\Controllers\Front\ArticleController::class, 'search'])->name('search');
Route::get('/announces', [\App\Http\Controllers\Front\ArticleController::class, 'announces'])->name('announces');
Route::get('/pages', [\App\Http\Controllers\Front\ArticleController::class, 'pages'])->name('pages');
...
// ユーザー別
Route::get('/users', [\App\Http\Controllers\Front\UserController::class, 'users'])->name('users.index');
Route::get('/users/{userIdOrNickname}', [\App\Http\Controllers\Front\UserController::class, 'user'])->name('users.show');
Route::get('/users/{userIdOrNickname}/{articleSlug}', [\App\Http\Controllers\Front\ArticleController::class, 'show'])->name('articles.show');
// 記事詳細・ダウンロード
...
Route::middleware(['restrict:invitation_code'])->group(function (): void {
    Route::get('/invite/{invitation_code}', [\App\Http\Controllers\Mypage\UserController::class, 'showInvite'])->name('user.invite');
    Route::post('/invite/{invitation_code}', [\App\Http\Controllers\Mypage\UserController::class, 'registration'])->name('user.registration');
});

Route::GET('/login', [\App\Http\Controllers\Mypage\UserController::class, 'showLogin'])->name('login');
Route::GET('/login/2fa', [\App\Http\Controllers\Mypage\UserController::class, 'showTwoFactor'])->name('two-factor.login');
Route::GET('/forgot-password', [\App\Http\Controllers\Mypage\UserController::class, 'showForgotPassword'])->name('forgot-password');
Route::GET('/reset-password/{token}', [\App\Http\Controllers\Mypage\UserController::class, 'showResetPassword'])->name('reset-password');

Route::middleware(['auth'])->group(function (): void {
    Route::get('/mypage/', [\App\Http\Controllers\Mypage\MypageController::class, 'index'])->name('mypage.index');
    Route::get('/mypage/verify-email', [\App\Http\Controllers\Mypage\MypageController::class, 'verifyEmail'])->name('mypage.verify-email');
    Route::get('/mypage/verify-required', [\App\Http\Controllers\Mypage\MypageController::class, 'verifyNotice'])->name('verification.notice');

    Route::middleware(['verified'])->group(function (): void {
        Route::get('/mypage/two-factor', [\App\Http\Controllers\Mypage\MypageController::class, 'twoFactor'])->name('mypage.two-factor');
        Route::get('/mypage/login-histories', [\App\Http\Controllers\Mypage\MypageController::class, 'loginHistories'])->name('mypage.login-histories');
        ...
        Route::get('/mypage/articles', [\App\Http\Controllers\Mypage\ArticleController::class, 'index'])->name('mypage.articles.index');
        Route::get('/mypage/articles/create', [\App\Http\Controllers\Mypage\ArticleController::class, 'create'])->name('mypage.articles.create');
        Route::post('/mypage/articles/create', [\App\Http\Controllers\Mypage\ArticleController::class, 'store']);
        Route::get('/mypage/articles/edit/{article}', [\App\Http\Controllers\Mypage\ArticleController::class, 'edit'])->name('mypage.articles.edit');
        Route::post('/mypage/articles/edit/{article}', [\App\Http\Controllers\Mypage\ArticleController::class, 'update']);
    });
});
