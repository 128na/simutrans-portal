...
### インターフェース化を検討してよい例外条件

新しい Repository を追加する際も本方針（具象クラス直DI、共通処理は `HasCrud` 等のトレイトで共有）
に従うこと。ただし次のような明確な理由が生じた場合に限り、インターフェース化を検討してよい:

- テストでモックしたい具体的な要求がある
- 将来的に DB 以外のデータソースへ切り替える可能性が具体的にある

上記に該当しない「念のため」の抽象化は導入しない。
