<div class="form-group">
    <label for="author"><span class="badge badge-danger mr-1">{{ __('Required') }}</span>{{ __('Author') }}</label>
    <input type="text" class="form-control" id="author" name="author" value="{{ old('author', $article->author ?? '') }}">
</div>

<div class="form-group">
    <label for="link"><span class="badge badge-danger mr-1">{{ __('Required') }}</span>{{ __('Link') }}</label>
...
<div class="form-group">
    <label for="thanks"><span class="badge badge-secondary mr-1">{{ __('Optional') }}</span>{{ __('Acknowledgments and Referenced') }}</label>
    <textarea class="form-control" id="thanks" name="thanks" rows="4">{!! e(old('thanks', $article->thanks ?? '')) !!}</textarea>
</div>

<div class="form-group">
    <label><span class="badge badge-secondary mr-1">{{ __('Optional') }}</span>{{ __('Agreement') }}</label>
    <div class="custom-control custom-checkbox">
        ...
        <label class="custom-control-label" for="agreement">{{ __('This article is published by author\'s permission or by author himself.') }}</label>
    </div>
</div>
