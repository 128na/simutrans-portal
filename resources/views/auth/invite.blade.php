...
<div class="mx-auto max-w-xl p-6 lg:px-8">
    ...
    <form action="{{route('user.registration', $invitee->invitation_code)}}" method="POST">
        ...
        <div class="flex flex-col gap-y-4 border-t border-c-sub/10 pt-6 lg:mx-0">
            <div>
                <label for="name" class="block text-sm/6 font-semibold text-c-main">名前</label>
                <p class="mt-2 text-sm text-c-sub">
                    記事やユーザー一覧に表示される名称です。
                </p>
                <div class="mt-2.5">
                    <input id="name" type="text" name="name" maxlength="100" autocomplete="name" value="{{old('name', '')}}" class="block w-full rounded-md bg-white px-3.5 py-2 text-base outline-1 -outline-offset-1 outline-c-sub placeholder:text-c-sub focus:outline-2 focus:-outline-offset-2 focus:outline-primary sm:w-128" />
                </div>
                @error('name')
            ...
            <div>
                ...
                <div class="mt-2.5">
                    <input id="password" type="password" name="password" minlength="11" value="{{old('password', '')}}" autocomplete="new-password" class="block w-full rounded-md bg-white px-3.5 py-2 text-base outline-1 -outline-offset-1 outline-c-sub placeholder:text-c-sub focus:outline-2 focus:-outline-offset-2 focus:outline-primary sm:w-128" />
                </div>
                @error('password')
                <div class="text-sm text-danger">{{ $message }}</div>
                @enderror
