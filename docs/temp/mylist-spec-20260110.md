...
- ユーザーとして、マイリスト一覧から公開URLをワンクリックでコピーして、共有できるようにしたい。
...
- MyListIndexPage（Blade基盤 + island）:
  - 自分のマイリスト一覧。カラム: タイトル/公開状態/アイテム数/更新日/操作（編集・削除・公開URLコピー）。
  - ソート: 更新日降順（デフォルト）。
  - ページネーション: 20件/ページ（初期値）。
  - **公開URLコピー機能**: 公開リスト（`is_public=true`）に対して「URLコピー」ボタンを表示
    - ボタン押下でクリップボードに `https://example.com/mylist/{slug}` をコピー
    - コピー成功時は「コピー済」に表示切り替え、トースト通知表示
    - 非公開リストではボタン非表示
...
- **公開リスト機能**: `GET /api/v1/mylist/public/{slug}` 実装済み
  - 認証不要の読み取り専用エンドポイント
  - 非公開記事は除外されて返却
- **並び替えAPI修正**: ルート定義順序を修正（具体的なパス→パラメータ）

**フロントエンド完了** ✅

- **Union型対応**: 公開/非公開記事の両方の構造に対応
- **型ガード関数**: 記事の公開状態を安全に判定
- **AddToMyListButton**:
  - 記事追加モーダル実装済み
  - リスト選択・新規作成・複数リストへの一括追加機能
  - 重複チェック機能
- **MyListItemsTable**:
  - メモ編集: 1行入力でセル内に収まる設計
  - 長いメモ: `truncate`で省略、ホバー時に全文表示
  - 非公開記事: バッジ表示、リンク無効化
  - 並び替え: 上下ボタンで位置変更（動作確認済み）
- **MyListIndexPage**: リスト一覧CRUD（動作確認済み）
- **MyListDetailPage**: アイテム管理UI（動作確認済み）
...
- コンポーネント ✅
  - `resources/js/features/mylist/AddToMyList.tsx`
  - **`resources/js/features/mylist/MyListTable.tsx` - マイリスト一覧テーブル** ✅
    - テーブル表示（タイトル/公開状態/アイテム数/更新日）
    - **公開URLコピー機能**: 公開リストに対して「URLコピー」ボタン表示 ✅ 実装完了
      - クリック時にクリップボードに `https://example.com/mylist/{slug}` をコピー
      - コピー成功時は「コピー済」表示に切り替え（2秒後に戻る）
      - トースト通知で成功/失敗をユーザーに通知
    - リスト編集・削除機能（モーダルUI）
...
- Bladeテンプレート ✅
  - `resources/views/mypage/mylists.blade.php` — マイリスト一覧ページ
  - `resources/views/mypage/mylist-detail.blade.php` — マイリスト詳細ページ
- Viteエントリーポイント更新 ✅
...
- `GET /api/v1/mylist/public/{slug}` ✅ 実装完了
  - `app/Http/Controllers/MyListController.php` の `showPublic()` メソッド
  - 認証不要で公開リストを取得可能
  - MyListService の `getPublicListBySlug()` を使用
  - 非公開記事の除外、セキュリティ考慮済み
  - レスポンス: 公開記事の完全情報（URL、サムネイル、投稿者、日付）のみ

### Test（未実装）

- Feature テスト（Laravel）: API エンドポイント、権限、非公開フィルタリング
  - テストクラス: `tests/Feature/MyListController/...Test.php`
  - 対象エンドポイント: CRUD、並び替え、公開取得
- Unit テスト（PHP）: サービス層ロジック、slug生成、並び替え
  - テストクラス: `tests/Unit/Services/MyListServiceTest.php`
  - 対象: slug生成ロジック、アイテム並び替え、フィルタリング
- Component テスト（Vitest）: React islands の動作検証
  - AddToMyList、MyListTable、MyListItemsTable の動作確認
...
- マイリスト追加後に何も出ないので挙動が分からない
  -> トースト機能を付ける？
- スマホ表示マイリスト追加モーダルで新しいリストを作成の作成ボタンが横からはみ出る
  ->レイアウト調整
  →done
- マイリスト追加モーダルでマイリストの件数が多いと上下にはみ出る
  -> スクロールさせる
  →done
- 公開マイリストのアイテムが表示されない（APIは200を返しているが、エラー表示になる）
  →done
