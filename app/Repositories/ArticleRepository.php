...
final class ArticleRepository
{
     ...
     */
    public function getForEdit(null|Article $article = null): Collection
    {
        return $this->model
            ->query()
    ...
    public function first(string $userIdOrNickname, string $slug): null|Article
    {
        $query = $this->model
            ->query()
            ->select(['articles.*'])
    ...
    public function findAllForAnalytics(
        ...
        $periodQuery = function ($query) use ($articleAnalyticsType, $period): void {
            $query
                ->select('article_id', 'count', 'period')
                ->where('type', $articleAnalyticsType)
