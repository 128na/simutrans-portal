...
@section('page-content')
  <div class="v2-page v2-page-lg">
    <div class="mb-12">
      ...
      <p class="text-c-sub">
        ログイン時に Google Authenticator
        などの多要素認証アプリによる追加認証を設定できます。
        <br />
      ...
      @switch(true)
        @case(session('status') === \Laravel\Fortify\Fortify::TWO_FACTOR_AUTHENTICATION_ENABLED || $errors->getBag('confirmTwoFactorAuthentication')->has('code'))
          ...
          <form action="{{ route('two-factor.confirm') }}" method="POST">
            ...
            <div>
              <button
                type="submit"
                class="v2-button v2-button-lg v2-v2-button v2-button-lg v2-button-primary w-full sm:w-64"
              >
                送信
              </button>
            </div>
        ...
        @case(session('status') === \Laravel\Fortify\Fortify::TWO_FACTOR_AUTHENTICATION_CONFIRMED || $user->two_factor_secret)
          ...
          <div>
            ...
            <textarea
              readonly
              class="v2-input w-full h-40 mb-4"
              onclick="this.select()"
            >
{{ implode("\n", $user->recoveryCodes()) }}</textarea
            ...
            >
              ...
              <div class="gap-x-2 flex">
                <button
                  type="submit"
                  class="v2-button v2-button-lg v2-button-sub"
                >
                  再生成
                </button>
              </div>
          ...
          <div>
            ...
            <form
              ...
              method="POST"
              class="js-confirm"
              data-text="二要素認証を無効化しますか？"
            >
              @csrf
              @method('DELETE')
              <button
                type="submit"
                class="v2-button v2-button-md v2-button-danger"
              >
                無効化
              </button>
            </form>
