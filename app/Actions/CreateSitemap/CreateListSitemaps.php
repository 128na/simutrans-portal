...
final class CreateListSitemaps
{
    ...
    private function getUrlSet(): Generator
    {
        yield 'user.xml' => User::has('articles')->cursor()
            ->map(fn (User $user): string => route('user', ['userIdOrNickname' => $user->nickname ?? $user->id]));
        yield 'category.xml' => Category::has('articles')->cursor()
            ->map(fn (Category $category): string => route('category', ['type' => $category->type->value, 'slug' => $category->slug]));
        yield 'tag.xml' => Tag::has('articles')->cursor()
            ->map(fn (Tag $tag): string => route('tag', $tag));
