...
<div class="form-group">
    <label for="status"><span class="badge badge-danger mr-1">@lang('Required')</span>@lang('Status')</label>
    <select class="form-control" id="status" name="status">
        @foreach (config('status', []) as $key => $name)
            <option value="{{ $key }}" {{ old('status', $article->status ?? config('status.draft')) === $key ? 'selected' : '' }}>@lang('statuses.'.$key)</option>
        @endforeach
    </select>
</div>

<div class="form-group">
    <label for="title"><span class="badge badge-danger mr-1">@lang('Required')</span>@lang('Title')</label>
    <input type="text" class="form-control" id="title" name="title" placeholder="Title" value="{{ old('title', $article->title ?? '') }}">
</div>

<div class="form-group">
    <label for="slug"><span class="badge badge-secondary mr-1">@lang('Optional')</span>@lang('Slug')</label>
    <input type="text" class="form-control" id="slug" name="slug" placeholder="Slug" value="{{ old('slug', $article->url_decoded_slug ?? '') }}">
    <small class="form-text text-muted">@lang('For article URL. If empty, the article title is used.')</small>
</div>

<div class="form-group">
    <label><span class="badge badge-secondary mr-1">@lang('Optional')</span>@lang('Thumbnail') </label>
    <div class="mb-2">
        ...
        <input type="hidden" id="thumbnail_id" name="thumbnail_id" value="{{ old('thumbnail_id', isset($article) ? $article->contents->thumbnail : '') }}">
    </div>
    <div>
