<form method="GET" action="{{ route('search') }}">
  <div
    class="flex flex-col gap-y-4 sm:flex-row sm:items-center sm:gap-x-4 mb-4"
  >
    <input
      type="search"
      name="word"
  ...
  <div class="mb-12">
    ...
    <el-disclosure
      id="search-options"
      {{ $showOptions ? '' : 'hidden' }}
      class="v2-accordion-content mt-2 block space-y-2"
    >
      <div class="mb-4">
        ...
        <div class="v2-checkboxes">
          @foreach ($options['postTypes'] as $postType)
            <label>
              <input
                class="v2-checkbox peer"
                type="checkbox"
                name="postTypes[]"
                value="{{ $postType->value }}"
                {{ in_array($postType->value, $condition['postTypes'] ?? []) ? 'checked' : '' }}
              />
            ...
            @foreach ($categories as $category)
              <label>
                <input
                  ...
                  name="categoryIds[]"
                  value="{{ $category->id }}"
                  {{ in_array($category->id, $condition['categoryIds'] ?? []) ? 'checked' : '' }}
                />
                <span class="v2-checkbox-label">
      ...
      <el-disclosure
        id="search-users"
        {{ $condition['userIds'] ?? [] ? '' : 'hidden' }}
        class="mb-4 block space-y-2 gap-y-2"
      >
        <div
          id="app-search-users"
          data-user-ids="@json($condition['userIds'] ?? [])"
        >
          Loading...
        </div>
      </el-disclosure>
      ...
      <el-disclosure
        id="search-tags"
        {{ $condition['tagIds'] ?? [] ? '' : 'hidden' }}
        class="mb-4 block space-y-2 gap-y-2"
      >
        <div
          id="app-search-tags"
          data-tag-ids="@json($condition['tagIds'] ?? [])"
        >
          Loading...
        </div>
      </el-disclosure>
