<template>
  <div>
    ...
    <b-form-group>
      ...
      <media-manager :id="article.id" v-model="article.contents.file" name="addon" type="Article"
    ...
    <b-form-group>
      ...
      <div v-show="fileSelected">
        <b-button :disabled="!hasReadme" @click="handleReadme" size="sm" variant="outline-secondary">
...
export default {
  ...
  computed: {
    ...
    hasReadme() {
      return (
        this.fileSelected && this.findAttachment(this.article.contents.file)?.fileInfo?.readmes
      );
    }
  },
  methods: {
    handleReadme() {
      const readmes = this.findAttachment(this.article.contents.file)?.fileInfo.readmes;
