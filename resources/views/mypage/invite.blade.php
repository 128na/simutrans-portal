    ...
    <div class="flex flex-col gap-y-4 border-t border-g2 pt-6 lg:mx-0">
        ...
        <div>
            ...
            <form id="generate" action="{{route('mypage.invite')}}" method="POST">
                ...
                <p class="text-lg font-semibold p-4 mb-4 text-sm text-g9 rounded-lg bg-gray-50 border border-g3 ">
                    {{route('user.invite',$user->invitation_code)}}
                </p>
                <div class="gap-x-2 flex">
                    <button type="submit" class="rounded-md bg-g5 px-4 sm:py-2 py-4 text-white cursor-pointer js-clipboard" data-text="{{route('user.invite',$user->invitation_code)}}">
                        リンクをコピー
                    </button>
            ...
            <table class="border-collapse whitespace-nowrap">
                <thead>
                    <tr>
                        ...
                        <th class="border border-g3 px-4 py-2 bg-g5 text-white">ユーザー名</th>
                    </tr>
                <tbody>
                    @forelse($user->invites as $invite)
                    <tr>
                        <td class="border border-g3 px-4 py-2">{{$invite->created_at->format('Y/m/d H:i:s')}}</td>
                        <td class="border border-g3 px-4 py-2">{{$invite->name}}</td>
                    </tr>
                    @empty
                    <tr>
                        <td class="border border-g3 px-4 py-2" colspan="2">履歴はありません</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
