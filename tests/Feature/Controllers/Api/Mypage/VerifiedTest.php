...
final class VerifiedTest extends TestCase
{
    ...
    public static function data(): \Generator
    {
        ...
        yield '添付ファイル削除' => [
            'deleteJson',
            fn(self $self): string => '/api/mypage/attachments/' . $self->createAttachment($self->user)->id,
        ];
        // 記事
        yield '記事投稿' => ['postJson', fn(self $self): string => '/api/mypage/articles'];
        yield '記事更新' => [
            'postJson',
            fn(self $self): string => '/api/mypage/articles/' .
            Article::factory()->create(['user_id' => $self->user->id])->id,
        ];
        // 分析
