...
class MetaOgpService
{
    ...
    public function frontArticleShow(User $user, Article $article): array
    {
        ...
        $descriptionString = is_string($description) ? $description : '';
        /** @var string|null $imageUrl */
        $imageUrl = $article->has_thumbnail ? $article->thumbnail_url : null;

        return [
            'title' => $article->title.' - '.Config::string('app.name'),
            'description' => $this->trimDescription($descriptionString),
            'image' => $imageUrl,
