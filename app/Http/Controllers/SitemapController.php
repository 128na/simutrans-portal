...
class SitemapController extends Controller
{
    public function index(): Response
    {
        ...
        // 記事に紐づいているタグすべて
        Tag::has('articles')->latest()->take(1000)->get()->each(function (Tag $tag) use ($sitemap) {
            $sitemap->add(Url::create(route('tags.show', $tag->name))->setPriority(0.6));
        });

        // 公開マイリストすべて
        MyList::where('is_public', true)->latest()->take(1000)->get()->each(function (MyList $myList) use ($sitemap) {
