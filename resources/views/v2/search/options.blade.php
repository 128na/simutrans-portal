<form method="GET" action="{{ route('search') }}">
    ...
    <div class="flex flex-col gap-y-4 sm:flex-row sm:items-center sm:gap-x-4">
        ...
        <button type="submit" class="rounded-md bg-brand px-4 py-2 text-white">
            検索
        </button>
    </div>
    <div>
        <button type="button" command="--toggle" commandfor="search-options" class="my-2 p-2 flex w-full items-center justify-between bg-gray-100">
            詳細条件
        ...
        <el-disclosure id="search-options" {{ $showOptions ? '' : 'hidden' }} class="mt-2 block space-y-2">
            ...
            <button type="button" command="--toggle" commandfor="search-users" class="my-2 p-2 flex w-full items-center justify-between bg-gray-100">
                ユーザー
                <svg viewBox="0 0 20 20" fill="currentColor" data-slot="icon" aria-hidden="true" class="size-5 flex-none in-aria-expanded:rotate-180">
                    <path d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" fill-rule="evenodd" />
                </svg>
